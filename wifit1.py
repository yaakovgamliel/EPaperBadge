...
def towif(im, outfn):
    ...
    f.seek(0)
    dat = f.read(width * height // 8 + struct.calcsize('HH'))
    want = width * height // 8 + struct.calcsize('HH')
    got = len(dat)
    assert got == want, '%s != %s' % (got, want)
    f = open(outfn, 'wb')
    f.write(dat)
    f.close()
    f = open(outfn, 'rb')
    assert len(f.read()) == want
class WIF:
    ...
    def init_image(self, fn, size=None):
        ...
        if fn is None:
            ...
            try:
                ...
                assert len(dat) == (height * width // 8), '%s != %s' % (len(dat), height * width // 8)
